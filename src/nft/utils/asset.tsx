...
export const generateTweetForAsset = (asset: GenieAsset): string => {
  ...
  }%20(${asset.collectionName})%20https://app.freeuniswap.org/nfts/asset/${asset.address}/${asset.tokenId}%20via%20@uniswap`
}
...
export const generateTweetForPurchase = (assets: UpdatedGenieAsset[], txHashUrl: string): string => {
  ...
  } with @Uniswap 🦄\n\nhttps://app.freeuniswap.org/nfts/${collectionUrl}\n${txHashUrl}`
  ...
  const tweetText =
    assets.length == 1
      ? `I just listed ${
          ...
          .join(', ')}. Buy it on @Uniswap at https://app.freeuniswap.org/${getAssetHref(assets[0])}`
      : `I just listed ${
          assets.length
        } items on @Uniswap at https://app.freeuniswap.org/nfts/profile\n\nCollections: ${mapAssetsToCollections(assets)
